...
//   const onSuccessPrice = (data) => {
   
//   }

//   const onErrorPrice = (error) => {
//    // errorNotification(error.response.data.message)
//     setErrorPrice(error.response.data.message)
//   }
  
//  //Get Courier Price -Info
//  const get_courier_price_info = (courier_cate_id, send_branch_id, receive_branch_id, weight) => {
//   return axiosPrivate.get(`${COURIERPRICEURL}/${courier_cate_id}/${send_branch_id}/${receive_branch_id}/${weight}`);
// }

//  const {data:data_courier_price_info, refetch:PriceRefetch} = useQuery(['combo-courier-price_info', get_courier_price_info], () => get_courier_price_info(trans_courier_cate_id, branch_id, trans_receiving_branch_id, trans_weight ),{
//   enabled: !!trans_weight,
//   onSuccess: onSuccessPrice,
...
setGetTransDate(formattedDate);
  ...
  // useEffect(() => {
  //   // if(trans_courier_cate_id && trans_receiving_branch_id && trans_weight){
  //   //   setTrans_getPrice("ok")
  //   // }else {
  //   //   setTrans_getPrice(null)
  //   // }

  //   if(trans_receiving_branch_id && branch_id){
  //      const get_trip = view_all_trips.find(trip => trip.from_branch_id == branch_id && trip.to_branch_id == trans_receiving_branch_id && trip.is_active == "YES")
  //      if(get_trip){
  //       setTrip_setup_id(get_trip.id)
       
  
  //       const get_courier_price = view_all_courier_prices.find(cp => cp.trip_setup_id == get_trip.id && cp.courier_cate_id == trans_courier_cate_id && cp.weight == trans_weight && cp.is_active == "YES")  

  //       //Check to get the max weight
  //       const getValues = view_all_courier_prices.filter(mcp => mcp.trip_setup_id == get_trip.id && mcp.courier_cate_id == trans_courier_cate_id && mcp.is_active == "YEST" )
  //       //const max = getValues.reduce((a, b) => { return Math.max(a.weight, b.weight) });
  //       const max = Math.max.apply(Math, getValues.map(function(p) { return p.weight; }))
   
  //       if(get_courier_price){
  //         if(trans_weight > max) {
  ...
  //           const getExtraPrice = view_all_extra_prices.find(ep => ep.trip_setup_id == get_trip.id && ep.courier_cate_id == trans_courier_cate_id)
  ...
  //        setTotalPrice(courierPrice + courierExtraPrice);


  //      }else{
  //       trip_setup_err("Please Ensure That Trip Is Setup")
  //      }
       
  //   }

  // },[trans_courier_cate_id, trans_receiving_branch_id, trans_weight])
  ...
  const handlePrices = (weight, receive_branch_id, courier_cate_id) => {
   ...
   //Get Id Type - Combo Box
//    const performGetIdType = (id) => {
//     return axiosPrivate.get(`${IDTYPES}`);
//   }

//   const onSuccessIdType = () => {}
//   const onErrorIdType = () => {}
//  const {data:data_idType} = useQueryData("combo-idType",performGetIdType,onSuccessIdType, onErrorIdType)
...
//       return axiosPrivate.get(`${BRANCHURL}/${id}`);
//     }
  
  //   const onSuccessBranch = () => {}
  //   const onErrorBranch = () => {}
  //  const {data:data_branch} = useQueryData("combo-branch",()=>performGetBranch(branch_id),onSuccessBranch, onErrorBranch)



       //Get Security Question - Combo Box
      //  const performGetSecurity = (id) => {
      //   return axiosPrivate.get(`${SECURITYQUES}`);
      // }
    
    //   const onSuccessSecurity = () => {}
    //   const onErrorSecurity = () => {}
    //  const {data:data_security} = useQueryData("combo-security",performGetSecurity,onSuccessSecurity, onErrorSecurity)



         //Get Courier Type - Combo Box
    // const performGetCustomer = (id) => {
    //   return axiosPrivate.get(`${CUSTURL}`);
    // }
  
  //   const onSuccessCustomer = () => {}
  //   const onErrorCustomer = () => {}
  //  const {data:data_cust} = useQueryData("combo-getCustomer",performGetCustomer,onSuccessCustomer, onErrorCustomer)



//    const performViewCust = (id) => {
//     return axiosPrivate.get(`${CUSTVIEWURL}/${id}`);
//   }
  
//    const onSuccessSenderView = (data) => {
//       // sender[0].fname = data?.data?.fname;
//       // sender[0].lname = data?.data?.lname;
//       // sender[0].contact = data?.data?.contact;
//       // sender[0].email = data?.data?.email;
//       // sender[0].id_type_id = data?.data?.id_type_id;
//       // sender[0].id_number = data?.data?.id_number;
//       // sender[0].branch_id = data?.data?.branch_id;
    
//     //console.log(data)
// };
// const onErrorSenderView = (error) => {
//    // console.log("error",error);
//    // navigate("/login", {state: {from: location}, replace:true})
// };


// const {isLoading:isLoadingS, data:dataS, isError:isErrorS, error:errorS, isFetching:isFetchingS, refetch:refetchS} = useSingleDataEnabled('get-courierSenderView','combo-getCustomer',custSender,performViewCust,onSuccessSenderView, onErrorSenderView,custSender);
...
// const onSuccessReceiverView = (data) => {
//   // receiver[0].fname = data?.data?.fname;
//   // receiver[0].lname = data?.data?.lname;
//   // receiver[0].contact = data?.data?.contact;
//   // receiver[0].email = data?.data?.email;
//   // receiver[0].id_type_id = data?.data?.id_type_id;
//   // receiver[0].id_number = data?.data?.id_number;
//   // receiver[0].branch_id = data?.data?.branch_id;

// //console.log(data)
// };
// const onErrorReceiverView = (error) => {
// // console.log("error",error);
// // navigate("/login", {state: {from: location}, replace:true})
// };


// const {isLoading:isLoadingR, data:dataR, isError:isErrorR, error:errorR, isFetching:isFetchingR, refetch:refetchR} = useSingleDataEnabled('get-courierReceiverView','combo-getCustomer',custSender,performViewCust,onSuccessReceiverView, onErrorReceiverView,custReceiver);
...
const  senderSchema = Yup.object({
  ...
  const transactionDetailSchema = Yup.object({
    ...
    const secSchema = Yup.object({
      ...
      // const performInTransactionDetail = (transaction_detail_data) => {
      //   return axiosPrivate.post(URL, transaction_detail_data, {
      ...
      //   const {mutate,data, isLoading, Loading, isError, error} = useMutationInitialInsert(performInTransactionDetail, onSuccess, onError);
    ...
    const onSubmit =  async (values, onSubmitProps)  => {
      ...
      // mutate(values);
       ...
       try{
        ...
        successNotification(`Courier Setup Successfully, Tracking Number Is`);
       // setTrackingNumber(data.data.tracking_number)
       // dispatch(setTrackingNumber({tracking_number:data[0].tracking_number}))
     
      // console.log(data)

       const track_rep = {
        track_id: data.id,
        send_b_id: data.branch_id,
        receive_b_id: data.receive_branch_id,
        sender_cust_id: data.sender_id,
        receive_cust_id: data.receiver_id,
        trip_id: data.trip_setup_id,
       }
      // localStorage.setItem("track_rep", JSON.stringify(track_rep))
       dispatch(setTrackRep({track_rep:track_rep}))
       setOpenModal(false)
        //navigate("/bill");
  ...
  return (
    ...
    <Modal show={openModal} onClose={() => setOpenModal(false)} size="4xl">
      ...
      <FormStep stepName="Examine Package" onSubmit={() => {}} className="flex max-w-full flex-col gap-4" validationSchema={transactionDetailSchema}>
          ...
          <div className='w-1/2'>
            <div className="mb-2 block">
              <Label htmlFor="trans_date" value="Date" />
            </div>
            <Field className="formInput" name="transaction_detail.trans_date">
                {
                  (props) => {
                ...
                <TextInput name="transaction_detail.trans_date" 
              type="date" 
              pattern="\d{4}-\d{2}-\d{2}"
              placeholder="Date" 
              required 
              value={form.values.transaction_detail.trans_date} 
              onChange={form.handleChange}
              onBlur={form.handleBlur}  />
            ...
            <ErrorMessage name="transaction_detail.trans_date" component={TextError} />
          </div>
